...
public class CyberScouterWordCloud {
    final static String WORDS_UPDATED_FILTER = "frcteam195_cyberscouterwordcloud_word_cloud_updated_intent_filter";
    ...
    void putWord(SQLiteDatabase db) {
        try {
            ...
            values.put(CyberScouterContract.Words.COLUMN_NAME_EVENT_ID, EventID);
            values.put(CyberScouterContract.Words.COLUMN_NAME_MATCH_ID, MatchID);
            values.put(CyberScouterContract.Words.COLUMN_NAME_MATCH_SCOUTING_ID, MatchScoutingID);
            values.put(CyberScouterContract.Words.COLUMN_NAME_SEQ, Seq);
            values.put(CyberScouterContract.Words.COLUMN_NAME_TEAM, Team);
            values.put(CyberScouterContract.Words.COLUMN_NAME_WORD_ID, WordID);
            values.put(CyberScouterContract.Words.COLUMN_NAME_DONE_SCOUTING, 0);
            values.put(CyberScouterContract.Words.COLUMN_NAME_UPLOAD_STATUS, UploadStatus.NOT_UPLOADED);
    ...
    public static void setDoneScouting(SQLiteDatabase db) {
        try {
            ...
            values.put(CyberScouterContract.Words.COLUMN_NAME_DONE_SCOUTING, 1);
            values.put(CyberScouterContract.Words.COLUMN_NAME_UPLOAD_STATUS, UploadStatus.READY_TO_UPLOAD);

            long newRowId = db.update(CyberScouterContract.Words.TABLE_NAME, values, null, null);
        } catch (Exception e) {
            e.printStackTrace();
        }
